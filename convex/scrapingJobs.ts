...
export const deleteJob = mutation({
  args: { jobId: v.id("scrapingJobs") }, // Changed 'id' to 'jobId'
  handler: async (ctx, args) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) {
      throw new Error("Not authenticated");
    ...
    const job = await ctx.db.get(args.jobId); // Changed args.id to args.jobId
    ...
    await ctx.db.delete(args.jobId); // Changed args.id to args.jobId
    return { success: true };
  },
});