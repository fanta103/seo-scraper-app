    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <CardContent className="p-6 relative">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="text-sm font-medium text-muted-foreground">Keywords Found</div>
            </div>
          ...
          <div className="flex-1">
            {seoReport?.keywords?.content_keywords && seoReport.keywords.content_keywords.length > 0 ? (
              <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
                {seoReport.keywords.content_keywords.map((keyword, index) => (
                  <Badge key={index} variant="secondary" className="text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300">
                    {keyword.keyword}
                  </Badge>
                ))}
              </div>
      ...
      <Card className="relative overflow-hidden border bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 group hover:shadow-md transition-all duration-300">
        ...
        <CardContent className="p-6 relative">
          ...
          <div className="flex-1">
            {seoReport?.inventory?.unique_domains && seoReport.inventory.unique_domains.length > 0 ? (
              <div className="space-y-1 max-h-24 overflow-y-auto">
                {seoReport.inventory.unique_domains.map((domain, index) => (
                  <div key={index} className="text-xs text-muted-foreground truncate bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded">
                    {domain}
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-xs text-muted-foreground">No domains found</div>
            )}
          </div>
        </CardContent>
      </Card>

      {/* 3. Unique Domains (Categories) */}
        ...
        <CardContent className="p-6 relative">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="text-sm font-medium text-muted-foreground">Unique Domains</div>
            </div>
          ...
          <div className="flex-1 space-y-1 max-h-24 overflow-y-auto">
            {Object.entries(seoReport?.inventory?.source_types || {})
              ...
              .map(([type, sources]) => (
                <div key={type} className="flex items-center justify-between text-xs bg-orange-50 dark:bg-orange-900/20 px-2 py-1 rounded">
                  <span className="capitalize text-muted-foreground">{type.replace(/_/g, " ")}</span>
                  <Badge variant="secondary" className="text-[10px] h-4 bg-orange-100 text-orange-700 dark:bg-orange-900/50">
                    {sources.length}
                  </Badge>
                </div>
              ))}
      ...
      {/* 4. Competitors (The Finished Section) */}
      {/* 4. Competitors (Schema Adjusted) */}
      ...
      {seoReport?.competitors && seoReport.competitors.length > 0 ? (
        seoReport.competitors.map((comp, index) => (
          <div key={index} className="flex items-center justify-between text-xs group/item">
            ...
            <Badge 
              variant="secondary"
              className="bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300 border-none shrink-0"
            >
              {/* Multiplying strength_score (0-10) by 10 to get percentage style */}
              {Math.round(comp.strength_score * 10)}%
            </Badge>
          </div>
        ))
      ) : (
